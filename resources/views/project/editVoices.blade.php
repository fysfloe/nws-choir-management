...
@section('body')
    {{ Form::open(['route' => ['project.saveVoices', $project], 'method' => 'POST', 'class' => 'edit-voices']) }}
    ...
    @if (count($project->voices) > 0)
        ...
        @foreach ($project->voices as $voice)
            <a
            href="{{ route('project.removeVoice', [$project, $voice]) }}"
            onclick="return confirm('{{ __('Do you really want to remove this voice from the project?') }}')"
            >
                <li
                    data-toggle="tooltip"
                    title="{{ __('Remove this voice from the project') }}"
            ...
            <select name="voices[]" class="form-control">
                @foreach ($voices as $voice)
                    <option value="{{ $voice->id }}"
                        @if ($project->voices->contains($voice)) data-selected="true" data-number="{{ $project->voices()->find($voice->id)->pivot->number }}" @endif
                    >
                        {{ $voice->name }}
                    </option>
                @endforeach
            </select>
            <small class="help-text text-muted" style="display: none">{{ __('This voice has already been added to the project.') }}</small>
        </div><!-- .form-group -->
